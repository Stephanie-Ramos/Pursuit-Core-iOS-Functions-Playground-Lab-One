...
func smallestValue(in nums: Array<Double>) -> Double { // call the internal parameter to nums, in order, to cancel an error in the for loop
    var smallestNum = nums[0] // nums[0] is better than a default 0.
    for num in nums {
        if  num < smallestNum { // int is converted to double when I declared smallestNum to double
            smallestNum = num // switched to cancel num is a constant error
        }
    }
    return smallestNum // return it outside of the for loop
}
